...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		\Require {Multiple-instance dataset $\bm{B} = \{\bm{B}_{1}, \dots, \bm{B}_{K}\}$, $\{L_{1}, \dots, L_{K} \}$, $L_{k} \in \{-1, +1\}$}
		\Ensure {Low-dimensional data representations $\bm{Z} \in \mathbb{R}^{N \times d}$}
		\While {Imputed labels change}
		\State {Compute low-dimensional data representations $\bm{Z}$ using S-LE or SE-Isomap with imputed instance labels}
		\State {\textbf{CLASSIFICATION}}
		\For {Every positive bag $\bm{B}_{k}^{+}$}
			\If {$\sum_{n \in N_{k}} \frac{l_{n} + 1}{2} == 0$}
