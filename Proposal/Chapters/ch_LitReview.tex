...
\subsection{Weak Learning on Manifolds}
...
The existing MIL manifold learning in the literature can be broken into two paradigms: LDA-based approaches and sparse, orthogonal matrix-based techniques \citep{Zhu2018MIDRSparsity}.  It should be noted that while this proposal addresses the problem of manifold learning  for instance-level classification, all MIL DR approaches in the literature were developed with bag-level classification in mind.  Additionally, all existing approaches are linear, meaning they may not work well if the underlying bag manifold exhibits curvature.  The existing approaches are briefly described.
...
\end{align}
\noindent
In \citep{Sun2010MIDR}, gradient descent was used to optimize the objective.  It was found that the newly developed method outperformed unsupervised instance-level dimensionality reduction approaches (applied to bags) for bag-level classification.  MIDR was later revisited by Zhu et al. where the optimization problem was reformulated using the intertial proximal alternating linearized minimization (iPALM) method \citep{Zhu2018MIDRSparsity}.  The advantage of this approach is that the problem variables can be managed sperately and updated effectively.  Additionally, the global convergence of MIDR was proved. 
...
In 2014, Chia et al. introduced Multiple-Instance Discriminant Analysis (MIDA) \citep{Chai2014MIDA}.  


  Citation Local Fisher Linear Discriminant Analysis (CLFDA) \citep{Kim2010LocalDRMIL}
