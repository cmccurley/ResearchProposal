...
%The elements of the alignment matrix can be found by an iterative summation 
%\noindent
%where $I_{n}$ represents the set of indices of the $k$-nearest neighbors of $\bm{x}_{n}$.  The low-dimensional data representations $\bm{Z}$ are found as the eigenvectors corresponding to the $d$ smallest nonzero eigenvalues of the symmetric matrix $\frac{1}{2}(\bm{B} + \bm{B}^{T})$ \citep{VanDerMaaten2009DRReview}.
...
\paragraph{Auto-Associative Models (Autoencoders)}

\paragraph{Nonlinear PCA}
...
In this paper, the SOM was applied to speech recognition, but made note of previous uses in robotics, control of diffusion processes, optimization problems, adaptive telecommunications, image compression, sentence understanding, and radar classification of sea-ice. \cite{Kohonen1990SOM} \\

The GHSOM has been applied to the areas of finance, computer network traffic analysis, manufacturing and image analysis (Palomo 2017). \cite{Rauber2002GHSOM,Palomo2017GHNG}

Thousands of applications, cross-modal information processing, text and document mining, gene expression data analysis and discovery, novelty detection, robotics and computer animation, prinicpal curve and surface discovery, visualization, \cite{Yin2007SOM}

hand-written word classification \cite{Chiang1997HandWrittenWords} and explosive hazard detection \cite{Frigui2009LandmineSOM}

A supervised adaptation of self-organizing maps has been proposed and is known as \textit{Learning Vector Quantization} (LVQ) \citep{Kohonen1995LVQ}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  GNG  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Growing Neural Gas (GNG)}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Elastic Maps  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Elastic Maps and Nets}
...
\paragraph{Independent Component Analysis (IDA)}
...
\begin{longtable}{ |p{0.25\textwidth}|p{0.6\textwidth}|p{0.08\textwidth}| } 
	...
	\hline
	PCA   & Preserves variance of data, global structure & 1901\\
	\hline
	MDS  & Preserves pairwise distances, global structure  & 1958\\
	\hline
	LDA  & Minimizes within-class distance and maximizes between-class distance, global structure  & 1936\\
	\hline
	NMF   & Decomposes data into low-dimensional coordinates and basis vectors, minimizes approximation error.  & 1994\\
	\hline
	NPP  & Linear variant of LLE, preserves local neighborhoods, local structure  & 2005\\
	\hline
	LPP  & Linear variant of LE, preserves local neighborhoods, local structure  & 2003\\
	\hline
	\multicolumn{3}{|c|}{\textbf{Nonlinear}} \\
	\hline
	\textbf{Method} & \textbf{Strategy} & \textbf{Intro.}\\
	\hline
	KPCA  & Performs PCA in a feature space defined by a kernel function, preserves variance of data, global structure  & 1999 \\
	\hline
	MVU  & Learns a kernel function to be used with KPCA that ``unfolds" the manifold, preserves variance of data, global structure  & 2004\\
	\hline
	Isomap  & Preserves geodesic distances, global structure  & 2000 \\
	\hline
	Sammon Mapping  & Preserves pairwise distances.  More emphasis is placed on samples that are close in the input space & 1969 \\
	\hline
	LLE  & Preserves local neighborhoods, local structure  & 2000 \\
	\hline
	Laplacian Eigenmaps  & Preserves local neighborhoods, local structure  & 2003 \\
	\hline
	Hessian LLE  & Minimizes curvature of high-dimensional manifold estimated from local neighborhoods & 2003 \\
	\hline
	LTSA  &  Learns manifold in local tangent spaces then aligns to form global coordinate system & 2002\\
	\hline
	Diffusion Maps  & Preserves diffusion distance  & 2006\\
	\hline
...
\begin{longtable}{ |p{2cm}|p{6cm}|p{2cm}|p{3cm}|  } 
	...
	\hline
	MIDR \newline (2010)   & Finds a sparse, orthogonal linear projection matrix optimized for bag-level logistic regression   &Linear, orthogonal projection  &  bag-level\\
	\hline
	MidLABS \newline (2010) &  Finds linear projection vector using LDA defined from bag-similarity kernel  & LDA   & bag-level\\
	\hline
	MIDA \newline (2014) &Learns linear projection vector using LDA defined from bag representative vectors & LDA &  instance-level\\
	\hline
	CLFDA \newline (2010)  &Learns linear projection matrix using local discriminant analysis defined from instance scatter.  Instance labels are provided as bag labels and refined. & LFDA &  instance-level\\
	\hline
	MI-FEAR \newline (2015) &  Incrementally leaves out feature and evaluates performance loss to provide feature score  & Feature selection & instance-level\\
	\hline
\end{longtable}
