...
\begin{align}
	\bm{m}_{k}(t+1) = \bm{m}_{k}(t) + \alpha(t) \cdot h_{ck}(t) \cdot [ \bm{x}(t) - \bm{m}_{k}(t) ] 
\end{align}
\noindent
...
\begin{align}
	h_{ck}(t) = \exp{\left( -\frac{||\bm{m}_{c} - \bm{m}_{k} ||^{2}}{\beta(t)} \right)}
\end{align}
\noindent
where the top expression represents the Euclidean distance between units $c$ and $k$ with $\bm{m}_{k}$ representing the 2-D or 3-D location of unit $k$ in the lattice.  The neighborhood kernel's bandwidth is typically initialized to a value which covers a majority of the input space and decreases over time such that solely the winner is adapted toward the end of the training procedure. \\
