...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\Ensure {Embedding coordinates $\bm{Z} \in \mathbb{R}^{N \times d}$}
...
Whereas linear PCA is limited to $d<D$ components, KPCA can use up to $N$ components.  Using a nonlinear feature embedding function along with the kernel trick provides for an elegant solution for capturing global nonlinear data structure.
