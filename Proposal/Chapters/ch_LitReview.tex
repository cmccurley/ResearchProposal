...
\noindent
where $\bm{I}$ is the $K \times K$ identity matrix and $\bm{e} = (1, \dots, 1)^{T}$, which is a $N \times 1$ \textbf{VERIFY SIZE of e and 0} vector.  The first constraint eliminates the trivial solution $\bm{Z} = \bm{0}$ (scaling) and the second constraint avoids the trivial solution $\bm{Z} = \bm{e}$ (uniqueness). By applying the Langrange multiplier method and using the fact that $\bm{L}\bm{e} = 0$, the low-dimensional data representations can be found by solving the generalized eigenvalue problem:
\begin{align}
	\bm{L}\bm{z} = \lambda \bm{D} \bm{z}
\end{align}
\noindent
The column vectors $\bm{z}_{1}, \dots, \bm{z}_{N}$ are the solutions of (\textbf{INCLUDE EQUATION NUMBER}), ordered to the corresponding non-zero eigenvalues in ascending order, $\lambda_{1} = 0 \leq \lambda_{2} \leq \dots \leq \lambda_{N} $. The embedding of the input samples is given by row vectors of the matrix $\bm{Z}$, or
\begin{align}
	\bm{x}_n \rightarrow \bm{z}_n = (z_{1}(n),\dots,z_{K}(n))^{T}
\end{align}
\noindent
where $K < N$ is the dimensionality of the embedded space.  From observation, it is clear that the embedding dimensionality is limited by the number of samples $N$.

\textbf{Finish discussion}
LE suffers from

Results vary highly on choice of neighborhood size and assumed intrinsic dimensionality,  However, choosing the size of $K$ or $\epsilon$ in advance can be very difficult. 

Despite these nuances, LE has been successfully applied in nonlinear dimensionality reduction tasks such as 

\textbf{End Discussion}

\paragraph{Supervised LE}
...
\end{align}
By defining matrix $\bm{B} = \gamma \bm{L}_{b} + (1 - \gamma)\bm{W}_{w}$, the problem becomes:
\begin{align}
...
\begin{align}
	\bm{B}\bm{z} = \lambda \bm{D}_{w}\bm{z}
\end{align}
