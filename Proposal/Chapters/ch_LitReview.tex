...
\paragraph{Auto-Associative Models (Autoencoders) and Nonlinear PCA} \label{sec:Autoencoders}

\textit{Autoencoder neural networks} fall into the categorization of neural architectures known as \textit{autoassociative networks} \citep{Rojas1996AssociativeNetworks}.  The goal of an autoencoder is, given an input sample, to produce an exact copy at its output.  Internally, the network has a hidden layer $\bm{h}$ that describes the latent code used to represent the input \citep{Goodfellow2016DeepLearning}.  Essentially, the network has two parts.  The first part represents the \textit{encoder} $\Phi_{encode}:\mathcal{X} \rightarrow \mathcal{Z}$, whereas the second part denotes the inverse function, or \textit{decoder} $\Phi_{decode}:\mathcal{X} \rightarrow \hat{\mathcal{X}}$.  A hidden layer in each part enables the network to perform nonlinear mappings \citep{Scholz2008NonlinearPCA}.  Early investigation of modern autoencoders was performed by LeCun, Bourlard, Kamp, Hinton and Zemel beginning in 1987 \citep{Goodfellow2016DeepLearning, Bengio2014RepLearningReview}.  
...
Standard Nonlinear PCA (NLPCA) is based on a multiple hidden layer autoencoder  \citep{Scholz2008NonlinearPCA}.

In fact, it has been shown that an autoencoder with only a single hidden layer will learn an equivalent transformation matrix to traditional PCA \citep{Bengio2014RepLearningReview}.

The autoencoder is tasked with learning the identity transform.  In other words, the desired output is the same as the input.  Because of the downsizing through the bottleneck layers, the network loses information through the nullspace of the network and cannot learn an exact identity transformation.  To perform dimensionality reduction, samples are passed through the encoder portion of the network and the low dimensional data representations are provided as the latent outputs $\bm{z}$ at the bottleneck layer.

Autoencoders can be used for anomaly detection in remote sensing, where the model can be trained on background data.  When presented a target sample, the autoencoder should not be able to reconstruct the sample.
