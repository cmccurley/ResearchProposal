...
While many manifold learning and dimensionality reduction models are regarded as defining a projection from a $D$- dimensional feature space into a lower $d$-dimensional space, a latent variable model is defined by a mapping \textit{from} the latent space \textit{into} the high-dimensional data space \citep{Bishop1998GTM}.  The goal of latent variable models in terms of low-dimensional feature learning is to discover the parameters of an invertible mapping from the latent generating distribution to the high-dimensional data distribution which maximized the likelihood of the data.  

Probabilistic methods provide two possible modeling paradigms for inferring latent variables, directed and undirected graphical models, which differ in their parameterizations of the joint distribution $p(\bm{x}_{n},\bm{z}_{n})$.  The remainder of this section focuses on directed graphical models, and specifically, on a particular formulation called \textit{Factor Analysis}.

\subsubsection{Factor Analysis (FA) and Probabilistic PCA (PPCA)} \label{sec:FA_and_PPCA}


Essentially, $p(\bm{x}_{n},\bm{z}_{n})$ defines a probabilistic model over the joint space of the latent variables, $\bm{z}_{n}$, and the observed data $\bm{x}_{n}$.  \textit{Directed latent factor models} separately parameterize the conditional data likelihood $p(\bm{x}_{n}|\bm{z}_{n}, \bm{\theta})$ and the prior $p(\bm{z}_{n})$ to construct the joint distribution, $p(\bm{x}_{n},\bm{z}_{n}) = p(\bm{x}_{n}|\bm{z}_{n}, \bm{\theta})p(\bm{z}_{n})$.
