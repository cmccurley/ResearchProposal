...
Nonlinear dimensionality reduction methods typically rely on the use of adjacency graphs.  These graphs represent data structure pooled from local neighborhoods of samples.  An overview of computational graphs as well as prominent methods for graph construction in manifold learning are presented. Additionally, geodesic distance approximation from pairwise distances is reviewed.

\paragraph{Terminology}
Spectral graph theory focuses on constructing, analyzing and manipulating graphs.  It has proved useful for graph visualization, spectral clustering, dimensionality reduction and numerous other applications in chemistry, physics, signal processing and computer science \citep{Shuman2013SignalProcessingGraphs}.  Dimensionality reduction methods are interested in analyzing relationships between samples defined on an undirected, weighted graph $\mathcal{G} = \{ \mathcal{V}, \mathcal{E}, \mathbf{W} \}$
