...
The score given to a sample is $l_{kn} = \bm{h}(\bm{x}_{kn})$ and $\bm{h}(\bm{x}_{kn}) = \sum_{t=1}^{T}\alpha_{t}h_{t}(\bm{x}_{kn})$

The probability that an instance is positive is given by the standard logistic function
\begin{align}
...
Following the idea of gradient boosting, the weight on each training instance is given as the derivative of the log-likelihood with respect to a change on the score given to the instance.  This is used to update the the strong classifier.  Pseudo-code for MIL-Boost is provided in Algorithm \ref{alg:mil_boost}.
...
Each round of boosting is a search for a classifier which maximizes $\sum_{kn} \alpha_{kn}\bm{h}(\bm{x}_{kn})$ where $\bm{h}(\bm{x}_{kn})$ is the score assigned to the sample by the weak classifier ($\bm{h}(\bm{x}_{kn}) \in \{-1, +1\}$ for a binary classifier).

It can be observed that as learning proceeds and the probability of the bag approaches the desired, the weight on the entire bag is reduced.  Within the bag, the examples are assigned a weight which is higher for samples with higher scores.  Thus the algorithm selects a subset of samples to assign a higher positive weight.  These samples control subsequent learning.
