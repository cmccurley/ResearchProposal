...
\end{align}
The symmetric matrix $\mathcal{H}$ gives an estimate of the curvature of the high-dimensional data manifold.  The low-dimensional embedding coordinates $\bm{Z}$ of the input data are given by the eigenvectors of the corresponding $d+1$ smallest eigenvalues of $\mathcal{H}$.

HLLE shares many of the characteristics of Laplacian Eigenmaps, except that it replaces the manifold Laplacian with the manifold Hessian.  Because of this, HLLE suffers from the same weaknesses as Laplacian Eigenmaps \citep{VanDerMaaten2009DRReview}.
