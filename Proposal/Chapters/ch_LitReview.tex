...
\end{align}
\noindent
This matrix represents the probability of a transition from one data sample to another in a single time-step.  Taking advantage of the Markov assumption, the forward transition probability matrix for $t$ time-steps $\bm{P}^{(t)}$ is given by $(\bm{P}^{(1)})^{t}$ \citep{Hajek2015RandomProcesses} and a \textit{diffusion distance} can be defined as
